...
struct SchoolInfo* get_school(const char* atptCode, const char* schulCode) {
  ...
  char url[512];
  snprintf(url, sizeof(url), "%s?Type=json&ATPT_OFCDC_SC_CODE=%s&SD_SCHUL_CODE=%s", API_URL_SCHOOLINFO, atptCode, schulCode);
  ...
  struct json_object* school_obj = json_object_array_get_idx(rows, 0);
